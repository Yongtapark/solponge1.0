...
<%@ page import="com.solponge.domain.JobScrap.JobScrapVO" %>
    ...
    <script language="JavaScript">
        ...
        function submitForm(clicked_id) {
            ...
            if (usercheck.length !== 0) {
                // var data1 = document.getElementById(infoname).value;
                var data2 = document.getElementById(companyName).value;
                // console.log('보낼 값: ' + data1 +'/' + data2);
                ...
                // let abandonedAnimal = JSON.stringify({ "infoname": data1, "companyName": data2});
                $.ajax({
                    type: "POST"
                    , url: "/scrap/company"
                    , contentType: "application/json"
                ...
                alert('찜 등록에 실패했습니다. 로그인 이후 사용해 주세요.');
                location.href = "/main";
            }
        }
...
<div id="container">
    <div class="container_left">
        ...
        <div id="row">
            ...
            <h3 id="newbook">
                새로 등록된 공고!!!<span><a href="">전체보기</a></span>
            </h3>
            <form id="my-form">
            <table id="Newjob8" style="font-size: 13px;">
                ...
                <c:forEach var="array" items="${arrayJobScrap}" varStatus="status" >
                    <input type="hidden" id="companyName_${array.companyName}" value="v_${array.companyName}"/>
                </c:forEach>
                <c:forEach var="NewJopInfo" items="${getJopInfoNewTop8List}" varStatus="status" >
                    ...
                    <input type="hidden" id="companyName_${NewJopInfo.infonum}" value="${NewJopInfo.companyname}"/>
                        <tr id="Newjob8td">
                            <td class="infonum" style="height: 54px">${NewJopInfo.companyname}<button id="${NewJopInfo.infonum}" type="button" onclick="submitForm(this.id)">Click</button></td>
                            <td style="height: 54px">${NewJopInfo.infoname}<br><span style="font-size: 10px">${NewJopInfo.ftaglist}</span></td>
